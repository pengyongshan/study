...
public class CrudExample {
    ...
    public static void createEphemeralSequential(CuratorFramework client, String path, byte[] payload)
            throws Exception {
    ...
    public static void setDataAsync(CuratorFramework client, String path, byte[] payload)
            throws Exception {
        //监听 this is one method of getting event/async notifications
        CuratorListener listener = (client1, event) ->
                System.out.println("CuratorListener:" + event.getType().name());
    ...
    public static void setDataAsyncWithCallback(CuratorFramework client, BackgroundCallback callback, String path, byte[] paylaod)
            throws Exception {
        //回调 this is another method of getting notification of an async completion
        client.setData().inBackground(callback).forPath(path, paylaod);
    }
